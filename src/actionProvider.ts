...
export class ActionProvider implements vscode.CodeActionProvider {
    ...
    vscode.ProviderResult<(vscode.Command | vscode.CodeAction)[]> {
        ...
        return context.diagnostics.map(diag => {
            ...
            const action = new vscode.CodeAction("Flag as okay", vscode.CodeActionKind.QuickFix)
            ...
            if (document.lineAt(flagPosition.start).firstNonWhitespaceCharacterIndex === flagPosition.start.character) {
                ...
                workspaceEdit.delete(document.uri, flagPosition.with(flagPosition.start, flagPosition.end.with(flagPosition.end.line, flagPosition.end.character + 1)))
            } else {
    ...
    findFlagPosition(document: vscode.TextDocument, range: vscode.Range, flag: string): vscode.Range | undefined {
        ...
        if (line.firstNonWhitespaceCharacterIndex === flagCharacterIndex) {
            result = line.range.with(new vscode.Position(line.lineNumber, flagCharacterIndex), new vscode.Position(line.lineNumber, flagCharacterIndex + 1))
        } else {
            result = line.range.with(new vscode.Position(line.lineNumber, flagCharacterIndex), new vscode.Position(line.lineNumber, flagCharacterIndex + 1))
        }
        return result
