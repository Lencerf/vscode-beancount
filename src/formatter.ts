...
export class Formatter {
    instantFormat(e:vscode.TextDocumentChangeEvent) {
        ...
        if (text === "\n" && rangeLength === 0) {
            ...
            if ( transArray != null ) {
                ...
                const r = new Range(new Position(line + 1, 0), new Position(line + 1, 0));
    ...
    alignSingleLine(line: number) {
        ...
        const contentBefore = ("s" + originalText.substring(0, amountArray.index)).trim().substring(1);
        ...
        const targetDotPosition = vscode.workspace.getConfiguration("beancount")["separatorColumn"] - 1;
        const whiteLength = targetDotPosition - contentBefore.length - (amountArray[0].length - 1)
        if (whiteLength > 0) {
            const newText = contentBefore + (new Array(whiteLength + 1).join(' ')) + amountArray[0] + contentAfterAmount
            const r = new vscode.Range(new vscode.Position(line,0), new vscode.Position(line, originalText.length))
